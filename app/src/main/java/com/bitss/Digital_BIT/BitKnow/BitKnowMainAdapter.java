...
public class BitKnowMainAdapter extends BaseAdapter {
  ...
  public BitKnowMainAdapter(Context context, List<QuestionModel> mData) {
  ...
  public View getView(int position, View convertView, ViewGroup arg2) {
    ...
    String url = mData.get(position).owner.iconUrl;
    try {
    ...
    String photoUrl = Constants.PHTOT_CLOUDSERVER_STRING + url;
    ...
    holder.name.setText(mData.get(position).owner.name);
    holder.time.setText(mData.get(position).date);
...
//    final String url3 = Constants.PHTOT_CLOUDSERVER_STRING + (String) mData.get(position).getUrl3();
    ...
    final QuestionModel data = mData.get(position);
...
//    }
    ...
    holder.content.setText((String) mData.get(position).content);

    String[] labels = new String[mData.get(position).tags.size()];
    int index = 0;
    for (TagModel tagModel : mData.get(position).tags) {
      labels[index++] = tagModel.tagName;
    }
    ...
    holder.answerNumber.setText(mData.get(position).answerNum + "人回答");
    return convertView;
  }
