...
public class QuestionDescribeActivity extends CustomBaseActivity {
  ...
  private static final int REQUEST_ADD_TOPIC = 1;
  ...
  private void intiOther() {
    ...
    describeContent = (EditText) findViewById(R.id.discrib_content);

    imageAdapter = new ImageAdapter(this, bitmaps);
    gv_image.setAdapter(imageAdapter);

    // 使描述问题文本框获得焦点
    describeContent.setFocusable(true);
    describeContent.setFocusableInTouchMode(true);
    describeContent.requestFocus();
    // describeContent.setOnFocusChangeListener(new OnFocusChangeListener()
    // {
  ...
  private void listens() {
    mTvRight.setOnClickListener(new OnClickListener() {
      ...
      public void onClick(View arg0) {
        String describe = describeContent.getText().toString().trim();
        if (describe.length() == 0 && picUri.size() == 0) {
          Toast.makeText(QuestionDescribeActivity.this, "请完善内容", Toast.LENGTH_SHORT)
                  .show();
          return;
        }

        Intent intent = new Intent(QuestionDescribeActivity.this,
                KnowAddTopicActivity.class);
        Bundle bundle = new Bundle();
        bundle.putString(INTENT_DESCRIBE, describe);
        bundle.putStringArrayList(INTENT_BITMAP, picUri);
        intent.putExtras(bundle);
        startActivityForResult(intent, REQUEST_ADD_TOPIC);
    ...
    gv_image.setOnListItemClickListener(new OnListItemClickListener() {
      ...
      public void onClick(View v, int position) {
        ...
        if (position == 0) {
          ...
          // Toast.makeText(QuestionDescribeActivity.this, account + "",
          // Toast.LENGTH_SHORT)
          // .show();
          if (bitmaps.size() < MAX_PIC_COUNT)
            takePhoto();
          else {
            Toast.makeText(QuestionDescribeActivity.this, "只能上传一张图片",
  ...
  private class ImageAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup arg2) {
      ...
      LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
              Utils.dp2pixel(QuestionDescribeActivity.this, 80),
              Utils.dp2pixel(QuestionDescribeActivity.this, 80));
      params.setMargins(Utils.dp2pixel(QuestionDescribeActivity.this, 5), 0,
              Utils.dp2pixel(QuestionDescribeActivity.this, 5), 0);
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (data != null && resultCode == RESULT_OK) {
      ...
      if (bitmap != null) {
        ...
        bitmaps.add(bitmap);
        imageAdapter = new ImageAdapter(QuestionDescribeActivity.this,
                bitmaps);
        gv_image.setAdapter(imageAdapter);
      }
