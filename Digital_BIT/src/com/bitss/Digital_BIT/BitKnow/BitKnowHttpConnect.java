...
public class BitKnowHttpConnect {
	...
	public BitKnowHttpConnect(Context context) {
		getClient = new DefaultHttpClient();
		mContext = context.getApplicationContext();
	}

	public void doGet(final String url,final String cloudurl, final BitKnowNetworkHandler mHandler) {
			...
			public void run() {
				...
				try {
					getClient.getParams().setParameter(CoreConnectionPNames.CONNECTION_TIMEOUT,
							CONNECTION_TIMEOUT);
					getClient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, SO_TIMEOUT);
					//请求云端服务器
					// 得到HttpGet对象
					HttpGet cloudrequest = new HttpGet(cloudurl);
					// 客户端使用GET方式执行请教，获得服务器端的回应response
					HttpResponse cloudresponse = getClient.execute(cloudrequest);
					// 判断请求是否成功
					if (cloudresponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
						Log.i(TAG, "connect success!");
						// 获得输入流
						String strResult = EntityUtils.toString(cloudresponse.getEntity());
	...
	public void doPost(final String url,final String couldurl ,final JSONObject se,
			final BitKnowNetworkHandler mHandler) {
